...
class CTCBeamDecoder(Decoder):
    ...
    def decode(
        ...
        for t in range(len(emissions)):
            ...
            for prev in self.beam:  # Loop over current candidates
                ...
                next_ = self.next_state(prev_state=prev, label=None, timestamp=None, cache=next_beam)
                ...
                for label in indices[t]:  # Loop over labels at time t
                    ...
                    if label == prev.label:
                        ...
                        next_n.p_nb = logsumexp(next_n.p_nb, prev.p_b + p + p_lm, prev.p_nb + p + p_lm)

            self.beam = sorted(next_beam.values(), key=lambda x: x.p_total, reverse=True)
    ...
    def next_state(
        ...
        if label is None:
            ...
            label_node = prev_state.label_node.child((label, timestamp))
            lm_node = prev_state.lm_node if prev_state.lm_node.is_root else prev_state.lm_node.parent
