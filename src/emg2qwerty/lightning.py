...
class CNNGRUCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        run_cnn: bool,
        bidirectional: bool,
        conv_channels: Sequence[int], # Fixed: Using the Plain CNN args
        conv_kernel_size: int,        # Fixed
        conv_dropout: float,          # Fixed
        optimizer: DictConfig,
        lr_scheduler: DictConfig,
        decoder: DictConfig,
    ) -> None:
        ...
        self.model = nn.Sequential(
            ...
            CNNGRUEncoder(
                ...
                conv_channels=conv_channels,       # Fixed: Passed down correctly
                conv_kernel_size=conv_kernel_size, # Fixed
                conv_dropout=conv_dropout,         # Fixed
            ),
            # (T, N, num_classes)
            nn.Linear(num_features, charset().num_classes),
            nn.LogSoftmax(dim=-1),
